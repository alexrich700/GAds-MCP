...
def test_expected_client_id(monkeypatch):
    monkeypatch.delenv("ADLOOP_EXPECTED_CLIENT_ID", raising=False)
    assert hosting_auth.expected_client_id() is None
    monkeypatch.setenv("ADLOOP_EXPECTED_CLIENT_ID", "  connector-xyz  ")
    assert hosting_auth.expected_client_id() == "connector-xyz"


# --- resolve_tenant (client_id pinning + tenant extraction) ----------------

def test_resolve_tenant_happy_path():
    assert hosting_auth.resolve_tenant(_token(subject="user-42"), "cid-123") == "user-42"


def test_resolve_tenant_rejects_unauthenticated():
    with pytest.raises(ToolError):
        hosting_auth.resolve_tenant(None, "cid-123")


def test_resolve_tenant_pins_client_id():
    # A token minted for a different connector must be refused.
    with pytest.raises(ToolError):
        hosting_auth.resolve_tenant(_token(client_id="some-other-connector"), "cid-123")


def test_resolve_tenant_skips_pin_when_expected_is_none():
    # Dev fallback: no pin configured => any verified token's tenant is used.
    assert hosting_auth.resolve_tenant(_token(client_id="whatever", subject="u1"), None) == "u1"


def test_resolve_tenant_falls_back_to_claims_sub():
    tok = _token(subject=None, claims={"sub": "user-from-claims"})
    assert hosting_auth.resolve_tenant(tok, "cid-123") == "user-from-claims"


def test_resolve_tenant_requires_a_subject():
    with pytest.raises(ToolError):
        hosting_auth.resolve_tenant(_token(subject=None, claims={}), "cid-123")
